...
把设置面板左侧导航栏中的分区（通用设置、模型、插件、Agent 预设等）归入**你自己创建**的可折叠分组，并可**隐藏**不常用的分区。

- 开启「侧边栏分区整理」开关后，导航栏中的分区就会按你的分组设置折叠。
- **分组由你自己创建**：在 **整理 → 分区显示设置** 里输入名称并点击「新建」创建分组；已创建的分组可删除。
- 每个分组的标题可点击展开/收起。
- 对每个分区可单独设置：
  - **显示/隐藏**：隐藏的分区不会在导航栏中显示（但内容仍然可通过其他方式访问）。
  - **分组归属**：手动指定该分区属于哪个分组（支持「不分组 / 任意你创建的分组」）。
...
4. 在「分区显示设置」里创建分组、调整每个分区的显示/隐藏与分组归属；
...
- **侧边栏分区整理**：`MutationObserver` 监听设置外壳的导航栏 `<nav.VOzbGW_nav .VOzbGW_navList>`，按分区标签文字匹配用户配置，用 `display:contents` 折叠容器对**用户自建分组**里的相邻分区折叠、用类名隐藏指定分区。没有内置的分组规则——分组完全由用户在「分区显示设置」里创建。因为 DSH 没有在 DOM 暴露分区内部 id，这里以**可见标签**作为匹配键，并对大小写不敏感。
...
- 用户自建分组：`SectionSettings` 组件（`navGroupNames` 创建/删除分组，`navGroups` 分配归属，`navHidden` 隐藏）
- CSS 覆盖：`CSS` 字符串
